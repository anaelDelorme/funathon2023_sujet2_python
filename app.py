...
if coordinates:
    ...
    with st.spinner('Chargement en cours...'):
       ...
       # query_prox = "SELECT * FROM (SELECT *  FROM rpg.parcelles  WHERE geom && ST_Expand(ST_SetSRID(ST_MakePoint("+ str(df.lat[0]) + ","+ str(df.lon[0])+"), 2154), "+ str(df.rayon[0])+")) subquery WHERE ST_Distance(ST_SetSRID(ST_MakePoint(" + str(df.lat[0]) +", " +str(df.lon[0])+"), 2154), geom) <= "+ str(df.rayon[0])+";"
        query_prox = "SELECT * FROM (SELECT *  FROM rpg.parcelles  WHERE ST_DWithin(ST_Transform(ST_SetSRID(ST_MakePoint("+ str(df.lon[0])+", "+ str(df.lat[0]) + "), 4326), 2154), geom, "+ str(df.rayon[0])+")) subquery WHERE ST_Distance(ST_Transform(ST_SetSRID(ST_MakePoint(" +str(df.lon[0])+", " + str(df.lat[0]) +"), 4326), 2154), geom) <= "+ str(df.rayon[0])+";"
