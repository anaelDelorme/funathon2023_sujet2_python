...
if coordinates:
    ...
    with st.spinner('Chargement en cours...'):
        ...
        query_prox = "SELECT * FROM (SELECT *  FROM rpg.parcelles  WHERE geom && ST_Expand(ST_SetSRID(ST_MakePoint(df.lon[0], df.lat[0]), srid), r)) subquery WHERE ST_Distance(ST_SetSRID(ST_MakePoint(df.lon[0], df.lat[0]), srid), geom) <= r;"
