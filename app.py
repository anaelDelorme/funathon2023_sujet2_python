...
if coordinates:
    ...
    with st.spinner('Chargement en cours...'):
        ...
        query_prox = "SELECT * FROM (SELECT *  FROM rpg.parcelles  WHERE geom && ST_Expand(ST_SetSRID(ST_MakePoint("+ str(df.lon[0]) + ","+ str(df.lat[0])+"), 2154), r)) subquery WHERE ST_Distance(ST_SetSRID(ST_MakePoint(" + str(df.lon[0]) +", " +str(df.lat[0])+"), 2154), geom) <= r;"
...
st.sidebar.write("*version : 2.0.7*")
